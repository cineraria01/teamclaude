...
<p align="center">
  <a href="https://github.com/sangrokjung/teamclaude/actions/workflows/tests.yml"><img src="https://github.com/sangrokjung/teamclaude/actions/workflows/tests.yml/badge.svg?branch=qjc%2Fresilient-routing" alt="테스트 실행 결과"></a>
...
인자가 환경변수보다 우선하기 때문에 `TEAMCLAUDE_PROVIDER=anthropic`을 명시해도 `codex`
...
변수를 그대로 따른다는 것 두 가지입니다. 이 테스트는 `status`만 실행하므로 위 `codex`
서브커맨드 경로는 커버하지 않습니다. 원본 `@karpeleslab/teamclaude`도 `teamclaude`
바이너리를 설치하니 둘을 같이 깔지 마세요.

계정은 **본인 것**을 쓰세요. 직접 결제한 클로드·ChatGPT 구독으로 로그인하면 됩니다.
이 도구는 그 로그인 사이를 오갈 뿐이지 한 자리를 여럿이 나눠 쓰라고 만든 물건이
아닙니다.
...
풀은 머신 한 대에서 돕니다. 다른 기기에서 사설 터널 같은 경로로 그 풀에 붙어도
결국 내 계정, 내 세션입니다. 런북도 그런 구성을 전제합니다.
지원하지 않는 쪽은 두 번째 사람입니다. 기준은 누구의 구독이 요청에 서명하느냐입니다.
...
**서드파티 클라이언트의 트래픽을 중계**합니다. 그 클라이언트에게 넘기는 풀은 **본인 구독**입니다.
이 표면은 이미 결제한 계정을 브라우저나 에디터에서도 쓰라고 있습니다. URL과 키를 남에게 돌리라고
있는 게 아닙니다. 둘 다 받은 사람은 누구든 그 로그인으로 쿼터를 씁니다.

기본 브랜치 빌드도 필요합니다(`npm i -g github:sangrokjung/teamclaude`). npm에 올라간 릴리스는
...
### Codex 모델 용량 부족 시 계정 재전환

`Selected model is at capacity`에 해당하는 `server_is_overloaded`가 응답하면
같은 모델·같은 요청을 다른 사용 가능한 계정으로 넘깁니다. 한도 초과로 계정을
전환한 직후에도 적용됩니다. 해당 요청에서 용량 부족을 반환한 계정은 다시
선택하지 않으며, 대안이 없으면 마지막 오류를 그대로 반환합니다.

HTTP 503 JSON 또는 답변·도구 출력 전의 비압축 SSE 오류에만 적용합니다.
일반 5xx·연결 끊김이나 이미 출력된 응답을 자동으로 중복 실행하지 않습니다.
오류 판별용 스트림 앞부분은 64 KiB까지만 보관하고, 정상 답변이 시작되면 즉시
기존 스트리밍으로 이어갑니다. 계정의 한도나 인증 상태는 변경하지 않습니다.

TUI에서 계정을 삭제하려면 계정 선택 후 `DELETE <계정 이름>`을 정확히 입력해야 합니다. Enter만으로는 삭제되지 않습니다.
