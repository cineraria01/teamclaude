...
TeamClaude is a transparent HTTP proxy that sits between Codex and the Anthropic API, managing multiple Codex (Max/Pro/API-key) accounts and rotating between them when one nears its session (5h) or weekly (7d) quota. Published as `@karpeleslab/teamclaude`.
...
`config.json` is gitignored; `config.example.json` is the template. To exercise the proxy end-to-end, start `server` in one terminal and `node src/index.js run` (or `eval $(node src/index.js env)` then `Codex`) in another.
...
Single CLI binary (`src/index.js`) dispatches subcommands; `server` boots a stable
public supervisor plus a replaceable proxy worker. The main source files keep
clear ownership:

- **`src/index.js`** — CLI dispatcher + all non-server commands (`stop`, `restart`, `import`, `login`, `env`, `status`, `accounts`, `remove`, `api`). Owns the **config-sync wiring** between the running server, the TUI, and external CLI invocations (see below), the **server-lifecycle helpers** (`findRunningServer`/`stopRunningServer`), and the supervisor that keeps the public listener bound, enforces public request admission by actual buffered bytes, and replaces a crashed/unresponsive worker without replaying an uncertain request.
- **`src/server.js`** — the worker HTTP proxy and request-forwarding loop (`forwardRequest`), including account acquisition (concurrency slot), bounded continuity recovery, model-aware failover, response/body deadlines, SSE streaming, and optional request logging.
...
- **`src/oauth.js`** — OAuth PKCE login, token refresh, profile fetch, and credential import from Codex. No proxy state here — pure functions.
...
4. Body is fully buffered so an explicit upstream 429 (a completed rejection) can be retried. `maxRequestBytes` and the aggregate actual-byte budget bound buffering; a supervised request reserves its public supervisor copy and worker copy separately. Hop-by-hop headers, `x-api-key`, and `accept-encoding` are stripped before forwarding (Node `fetch` auto-decompresses, so `content-encoding`/`content-length` are also dropped on the way back).
5. Account acquired via `acquireAccount()` (reserves one of the account's concurrency slots — see Concurrency below); OAuth token refreshed if expiring within 5 min. The slot is released in the request's `finally`; a failover releases `ctx.held` via `releaseHeld()` before recursing, while a 401 same-account refresh-retry keeps the slot.
...
7. **Transient network errors** (`ECONNRESET`/`ETIMEDOUT`/`fetch failed`) fail over only for replay-safe methods. An unsafe POST is never hiddenly replayed and receives a complete 502 explaining that dispatch was ambiguous.
8. All accounts temporarily unavailable → continuity mode waits within its total deadline and then returns the saved/computed 429. Disabled-only and unrecoverable auth-error fleets terminate immediately.

### Quota tracking (account-manager.js)

Two header families drive rotation, normalized into one model:
- **Unified** (`anthropic-ratelimit-unified-5h/7d-utilization` + `-reset`) — Codex Max/Pro. Utilization is already `0–1`.
- **Standard** (`anthropic-ratelimit-tokens/requests-*`) — API-key accounts; utilization is derived as `1 - remaining/limit`.

**Model-scoped weekly windows** (`anthropic-ratelimit-unified-7d_<label>-*`, currently `7d_oi` for Fable/Mythos) are parsed generically into `quota.modelWeekly` and surfaced in the TUI/status. A window pre-blocks selection only when its utilization and future reset are both finite/fresh and utilization is at or above `switchThreshold`, and only for the matching model family; unknown, partial, expired, and ready windows stay eligible so the request can refresh them, while Opus/Sonnet/Haiku routing is unaffected. Model-scoped values are not restored from quota snapshots. A live labeled model 429 is classified before general exhaustion so it does not globally throttle an otherwise usable account. The warm-up template may upgrade once to a top-tier shape whose own response carried a model window; advisor-nested model responses cannot seed that template.
...
- **Forced fleet re-measure (TUI `R`)**: `server.refreshQuotaAll()` refreshes lapsed tokens, then probes every idle non-error account, including disabled, measured, throttled, and near-quota rows, so the dashboard reflects usage spent outside this proxy. In-flight accounts are skipped because their active response will refresh them. It renews convergence budgets and reports measured/target counts honestly, or `-1` before any accepted template exists. It is deliberately not exposed over HTTP because it spends real upstream requests.
...
- **Memory bounds (localhost auth is skipped, so any local process could flood):** the public supervisor and worker reserve actual received bytes against `maxBufferedRequestBytes` instead of multiplying request count by the per-request maximum. Supervised bodies account for both copies; direct fixed-length worker bodies use one preallocated copy, while chunked bodies reserve the temporary concat peak before settling back to one copy. `maxRequestBytes` still caps each request (default 32 MiB → `413`), and the supervisor also keeps a bounded request/overflow admission count.
...
Related gotcha: **`expiresAt` may arrive in seconds or milliseconds.** OAuth endpoints return seconds; Codex credentials use milliseconds. Always pass through `normalizeExpiresAt` (oauth.js) — assuming one unit was a recurring bug.

## Other intentional behaviors

- **`run` deliberately sets only `ANTHROPIC_BASE_URL`, not `ANTHROPIC_API_KEY`** — this keeps Codex in subscription mode (full model access) while routing through the proxy, which accepts the client's own OAuth token from localhost.
