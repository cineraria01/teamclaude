...
6. 전체 검증이 남기는 자동 시작 서버를 정리한다. supervisor는 기록된 부모가 종료되어 실제 부모가 PID 1로 바뀐 경우만 허용하며 PID·시작 시각·명령·리스너 소유권·인증된 lifecycle 일치는 계속 필수다. worker의 부모 검사와 신호 직전 재검사는 유지한다. 시작에 실패한 경우에는 그 호출이 직접 생성한 daemon에 종료 신호를 보낸다.
