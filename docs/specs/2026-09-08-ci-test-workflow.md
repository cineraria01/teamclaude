...
4. GitHub Actions는 push, pull_request, 수동 실행에서 전체 Node 테스트를 직렬 실행한다. 저장소 읽기 권한만 부여하며 시크릿·외부 AI 계정이 필요하지 않다.
...
기존 PR #34 브랜치에서 최신 기본 브랜치를 통합한다. hash gate는 유지하며 evidence를 갱신한다. GitHub Actions 자체 배지로 정적 숫자를 교체한다. 로컬 검증·독립 리뷰 후 기존 PR을 갱신하고 Actions 결과를 확인한다. 되돌릴 때는 이 작업의 커밋만 revert하여 이전 배지와 CI 상태로 복구한다. 운영 서비스 변경은 없으므로 서비스 rollback은 불필요하다.
