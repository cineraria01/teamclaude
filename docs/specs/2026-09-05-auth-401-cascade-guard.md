...
Each of them was marked `status='error'`, `errorReason='auth-revoked'`, all
...
4. Un-parking is guarded by ownership plus observable state. Each park stamps
   ...
   `errorReason === 'auth-revoked'`, `_errorFromRefresh === false`). A later
   ...
   so an `auth-revoked` park survives it untouched. Gating on the generation
...
2. One account 401s, another serves → the 401 account is still parked
   (`status='error'`, `errorReason='auth-revoked'`) and the client gets 200.
