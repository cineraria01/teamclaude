...
// with 'auth-revoked'. A direct probe afterwards proved those credentials were
...
// ('auth-revoked' would also cost it the refresh-sweep self-healing that
...
test('a cascade leaves a refresh-failed account parked with its own reason', async () => {
  ...
  try {
    ...
    assert.equal(am.accounts[2].errorReason, 'refresh-failed', 'the cascade must not relabel it auth-revoked');
...
// `_credentialGeneration` but deliberately does NOT heal an 'auth-revoked'
