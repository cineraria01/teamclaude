...
function isLineTerminator(ch) { return LINE_TERMINATORS.has(ch); }
...
export function tokenizeJs(source) {
  ...
  let i = 0;
  let line = 1;
  const countLine = at => isLineTerminator(source[at]) && !(source[at] === '\n' && source[at - 1] === '\r') ? 1 : 0;
  ...
  while (i < source.length) {
    const ch = source[i];
    if (isLineTerminator(ch)) { line += countLine(i); i++; continue; }
    ...
    if (ch === '/' && source[i + 1] === '*') {
      ...
      for (let at = i; at < end; at++) line += countLine(at);
      i = end + 2;
      continue;
    }
    if (ch === '"' || ch === "'") {
    ...
    if (ch === '/' && regexAllowed()) {
      ...
      while (i < source.length) {
        const c = source[i];
        if (c === '\\') { i += 2; continue; }
        if (isLineTerminator(c)) throw new Error(`unterminated regex at line ${line}`);
    ...
    if (punct === '(') {
      ...
      const beforePrev = tokens[tokens.length - 2];
      const property = beforePrev && beforePrev.type === 'punct' && (beforePrev.value === '.' || beforePrev.value === '?.');
      const forAwait = isIdent(prev, 'await') && isIdent(beforePrev, 'for')
        && !isPunct(tokens[tokens.length - 3], '.') && !isPunct(tokens[tokens.length - 3], '?.');
      parenKinds.push(forAwait || Boolean(prev && prev.type === 'ident' && CONTROL_FLOW_PAREN.has(prev.value) && !property));
    }
    if (punct === ')') {
...
function isClassMemberName(tokens, match, index) {
  if (!insideClassBody(tokens, match, index)) return false;
  let k = index - 1;
  if (isPunct(tokens[k], '*')) {
    if (!isPunct(tokens[index + 1], '(') && !isPunct(tokens[index], '[')) return false;
    k--;
  }
  while (isIdent(tokens[k], 'static') || isIdent(tokens[k], 'get')
      || isIdent(tokens[k], 'set') || isIdent(tokens[k], 'async')) k--;
  return isPunct(tokens[k], '{') || isPunct(tokens[k], ';') || isPunct(tokens[k], '}');
}
...
function classifyParamToken(tokens, match, index, fn) {
  ...
  if (isClassMemberName(tokens, match, lo)) return 'key';
    ...
    if (token.type === 'punct') {
      ...
      if (OPENERS[token.value] || token.value === '${') {
        ...
        if (token.value === '{' || token.value === '[') {
          if (token.value === '[' && isClassMemberName(tokens, match, k)) return 'read';
...
export function auditRetryCycle(source, {
  ...
  tokens.forEach((token, index) => {
    ...
    const bareReset = (isPunct(tokens[index - 1], '{') || isPunct(tokens[index - 1], ';'))
      && isPunct(tokens[index + 3], ';');
    writes.push({ line: token.line, inHelper, bareReset, text: argText(tokens.slice(index, index + 3)) });
  ...
  if (helperRange) {
    ...
    if (helperWrites.length !== 1 || helperWrites[0].text !== `${param} = 0` || !helperWrites[0].bareReset) {
